...
def validate(article, response, packet, now):
    ...
    if packet.get('statistical_context',{}).get('scope')=='mlb_team_playoff_outlook':
        # Some responses use the official numeric team ID instead of our
        # prefixed record ID. This is an exact alias, never an inferred source.
        aliases={str(row['team_id']):row['id'] for row in statistics}
        article['market_ids']=[aliases.get(str(identifier),identifier) for identifier in article['market_ids']]

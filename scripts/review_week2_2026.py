#!/usr/bin/env python3
"""Audit the actual archived Week 2 pages against saved 2026 results.

No model refit, paid API calls, or inference of missing results as zero.
Run from repository root: .venv/bin/python scripts/review_week2_2026.py
"""
import hashlib
import json
import re
...
def archive(page):
    global REF
    REF = '13c730d'
    early, early_source = single_archive(page)
    REF = '5a4f3bf'
    late, late_source = single_archive(page)
    early = early[~early.game_id.isin(late.game_id)].copy()
    early['archive_ref'] = '13c730d'
    late['archive_ref'] = '5a4f3bf'
    return pd.concat([early, late], ignore_index=True), {'snapshots': [early_source, late_source], 'rule': 'Latest of these two pregame snapshots per game: Thursday for DET-BUF, Sunday for other 15 games.'}
...
def grade(props, stats, schedule):
    d = props.copy()
    d['stat_game'] = '2026_02_' + d.away_team.map(TEAMS) + '_' + d.home_team.map(TEAMS)
...
def main():
    ...
    schedule = pd.read_csv('reports/week2-2026/source/schedule_2026.csv').query('season == 2026 and week == 2').copy()
    stats = pd.read_parquet('reports/week2-2026/source/stats_player_week_2026.parquet').query('season == 2026 and week == 2 and season_type == "REG"')
    ...
    priced = pd.read_csv('reports/week2-2026/source/priced_week2_2026.csv')
    totals = pd.read_csv('reports/week2-2026/source/team_totals_week2.csv')
    ...
    source_paths = ['reports/week2-2026/source/schedule_2026.csv','reports/week2-2026/source/stats_player_week_2026.parquet',
        'reports/week2-2026/source/priced_week2_2026.csv','reports/week2-2026/source/team_totals_week2.csv']
    summary = dict(season=2026, week=2, dates='September 17–21, 2026', sources=[board_source,top_source]+
    ...
    Path('docs/blog/week-2-2026-review-data.json').write_text(json.dumps(public,indent=2,allow_nan=False)+'\n')

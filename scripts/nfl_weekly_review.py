...
def totals_metrics(archive,schedule,season,week):
    ...
    games['model_units']=0.0
    for idx,row in games.iterrows():
        if row.model_result=='loss':games.loc[idx,'model_units']=-1
        elif row.model_result=='win':
            price=row.best_over_price if row.model_pick=='over' else row.best_under_price
            games.loc[idx,'model_units']=american_profit(price) if pd.notna(price) else np.nan
...
def review_week(season,completed,preview,root=ROOT):
    ...
    chart(outdir/'props.svg',f'Week {completed}: prop shortlist by market','One archived selection per player / game / market',

...
def render(df: pd.DataFrame, season: int, week: int) -> str:
    ...
    for _, r in df.iterrows():
        ...
        best_over = "—" if pd.isna(r.get("best_over")) or not r.get("best_over") else str(r.best_over)
        best_under = "—" if pd.isna(r.get("best_under")) or not r.get("best_under") else str(r.best_under)

...
(async()=>{
  ...
  try{
    ...
    for(const width of [390,768,1280,1440,1920]){
      ...
      for(const route of ['/nba/','/nba/props/','/nba/totals/','/nba/top.html','/nba/methods.html','/']){
        await p.goto(base+route);if(route.startsWith('/nba'))await p.waitForFunction(()=>!document.getElementById('feed-status').textContent.includes('Enable JavaScript'));
        assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false,`Overflow ${width} ${route}`);
        assert.equal(await p.locator('.nba-sport').count(),1);
      ...
      if(await p.locator('.fv-burger').isVisible())await p.locator('.fv-burger').click();
      await p.locator('.nba-sport button').click();await p.getByRole('link',{name:'NBA Overview',exact:true}).click();assert(p.url().endsWith('/nba/'));
      if(width===390||width===1440)await p.screenshot({path:`/tmp/fv-nba-${width}.png`,fullPage:true});
      await p.close();
    }
    ...
    const row={event_id:'g',game:'Away @ Home',commence_time:future,quoted_at:now.toISOString(),player:'Example Player',market:'player_points',market_label:'Points',line:20.5,side:'Over',price:-110,book:'a',book_label:'Book A',book_probability:.5238,fair_probability:.5,consensus_probability:.5,paired_books:2,baseline_mean:null};
    ...
    await p.route('**/nba/data/latest.json',r=>r.fulfill({json:fixture}));
    await p.goto(base+'/nba/props/');await p.waitForSelector('.prop-card');assert.equal(await p.locator('.prop-card').count(),1);
    ...
    assert.deepEqual(errors,[]);console.log('PASS: NBA nav/layout at five widths, populated props filters, best-book selection, error/stale suppression.');
